...
router.post('/register', upload.single('profilePicture'), async (req, res) => {
  ...
  if (!name || !ownerName || !location || !contactNumber || !email || !password) {
  ...
  try {
    ...
    res.status(201).json({ 
      ...
      restaurant: {
        ...
        profilePicture: newRestaurant.profilePicture
      }
    });
  } catch (err) {
    console.error(err);
...
// ✅ Public endpoint for customers to view all restaurants
